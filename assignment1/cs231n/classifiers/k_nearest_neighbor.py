...
class KNearestNeighbor(object):
    ...
    def compute_distances_no_loops(self, X):
        ...
        a_b_2 = 2 * X.dot(self.X_train.transpose())

        X_square = np.sum(X_square, axis=1)
        X_train_square = np.sum(X_train_square, axis=1)

        X_square = np.array([X_square] * num_train).transpose()
        X_train_square = np.array([X_train_square] * num_test)

        #print(X_square.shape)
        #print(X_train_square.shape)
        #print(a_b_2.shape)

        dists = np.sqrt(X_square + X_train_square - a_b_2)
