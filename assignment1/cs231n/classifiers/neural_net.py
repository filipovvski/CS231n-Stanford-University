...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        scores = scores - np.reshape(maxes, (-1, 1))

        sum = np.sum(np.exp(scores), axis=1)
        softmax = np.exp(scores)/np.reshape(sum, (-1, 1))

        good_class_score = softmax[np.arange(N), y]
        loss = np.sum(-np.log(good_class_score))
        ...
        #W2 grad
        #print(softmax.shape, W2.shape)
        dfdW2 = ReLU.T.dot(softmax)
        dfdW2 += reg * 2 * W2
        grads['W2'] = dfdW2

        #b1 grad
        dfdReLU = softmax.dot(W2.T)
        dfdh1 = dfdReLU * (h1 > 0)
        dfdb1 = np.sum(dfdh1, axis=0)
        grads['b1'] = dfdb1
